...
import torch
import pytest
import nanoproof.flash_attention as fa_module
from nanoproof.flash_attention import flash_attn, HAS_FA3
from nanoproof.engine import KVCache
...
class TestFA3VsSDPA:
    ...
    def test_full_context(self):
        ...
        def run():
            return flash_attn.flash_attn_func(
                q, k, v, causal=True, window_size=(-1, -1)
            )
    ...
    def test_kvcache_single_token(self):
        ...
        def run():
            ...
            cache_seqlens = torch.full(
                (B,), T_prefill, dtype=torch.int32, device=self.DEVICE
            )
            return flash_attn.flash_attn_with_kvcache(
                q_single,
                k_cache,
    ...
    def test_kvcache_single_token_sliding_window(self):
        ...
        print(
            f"single_token_sliding_window: max_diff={max_diff:.6f}, mean_diff={mean_diff:.6f}"
        )
    ...
    def test_backward_gradients_match(self):
        ...
        assert_close(y_fa3, y_sdpa, "backward_output")
        assert_close(q_grad_fa3, q_grad_sdpa, "q_grad", atol=0.05, rtol=0.05)
        assert_close(k_grad_fa3, k_grad_sdpa, "k_grad", atol=0.05, rtol=0.05)
        assert_close(v_grad_fa3, v_grad_sdpa, "v_grad", atol=0.05, rtol=0.05)
...
class TestSDPAOnly:
    ...
    def test_backward(self):
        ...
        B, T, H, D = 2, 32, 4, 16
        q = torch.randn(
            B, T, H, D, device=self.DEVICE, dtype=self.DTYPE, requires_grad=True
        )
        k = torch.randn(
            B, T, H, D, device=self.DEVICE, dtype=self.DTYPE, requires_grad=True
        )
