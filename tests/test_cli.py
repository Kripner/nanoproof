...
def simulate_collection_phase(monitor, duration: float = 20.0):
    ...
    log(
        f"Collection complete: {samples} samples from {proofs_successful} proofs",
...
def simulate_training_phase(monitor, num_steps: int = 10):
    ...
    for step in range(num_steps):
        ...
        monitor.set_replay_buffer_size(
            monitor.replay_buffer_size + random.randint(0, 20)
        )

        log(
            f"Step {monitor.step + step}: loss={loss:.6f}, tokens={tokens}",
            component="Train",
        )
...
def simulate_eval_phase(monitor, step: int):
    ...
    log(
        f"MiniF2F: {minif2f_solved}/{minif2f_total} ({minif2f_solved / minif2f_total * 100:.1f}%)",
        component="Eval",
    )
    ...
    log(
        f"LeanWorkbook: {lwb_solved}/{lwb_total} ({lwb_solved / lwb_total * 100:.1f}%)",
        component="Eval",
    )
...
def main():
    ...
    actor_thread = threading.Thread(
        target=simulate_local_actors, args=(monitor, 16), daemon=True
    )
    actor_thread.start()
    ...
    gpu_thread = threading.Thread(
        target=simulate_gpu, args=(monitor, 0, "NVIDIA A100 40GB"), daemon=True
    )
    gpu_thread.start()
