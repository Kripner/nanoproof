...
# Mock openproof.common to avoid import errors and control get_dist_info
sys.modules["openproof.common"] = MagicMock()
...
sys.modules["openproof.muon"] = MagicMock()
sys.modules["openproof.adamw"] = MagicMock()

from openproof.model import Network, NetworkConfig, ValueHead
...
def test_network():
    ...
    # We mocked openproof.common, so we need to set the return value on that mock
    # Note: model.py imports get_dist_info FROM openproof.common.
    # Since we mocked sys.modules["openproof.common"] BEFORE import, 
    # the imported get_dist_info in model.py is the mock.
    # However, we need to access the SAME mock object to configure it.
    # We can access it via sys.modules["openproof.common"].get_dist_info
    sys.modules["openproof.common"].get_dist_info.return_value = (False, 0, 0, 1)
